...
|start_mask|表示する期間(開始日付)|
|end_mask|表示する期間(終了日付)|
...
|表示期間|WBSの表示期間を指摘できる<br>項目の開始日が表示期間に入っているものが描画対象となる|
|テキスト編集|クリックすると編集エリアの表示／非表示が切り替わる<br>編集エリアのコンボボックスを切り替えると編集モードが切り替わる<br><br>＜新規追加モード＞<br>1. テキストエリアに項目名を入力<br>2. 「適用」をクリック<br><br>＜項目名変更モード＞<br>1. テキストエリアの項目名を変更する<br>　※項目数、各項目の階層の値の変更は禁止<br>　※項目名がかぶらないようにすること<br>2. 「適用」をクリック<br><br>＜階層変更、並び替え、削除モード><br>1. テキストエリアの項目の階層、順番を変更する<br>　※削除したい項目は行を削除<br>2. 「適用」をクリック<br><br>全モードでの共通事項<br>・1行1項目<br>・階層は項目名の前に「*」を階層数-1個追加することで指定する|
